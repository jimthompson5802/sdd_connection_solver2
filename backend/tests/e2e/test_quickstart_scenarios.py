...
class TestQuickstartJourney2_OllamaIntegration:
    ...
    # @patch('src.services.ollama_service.OllamaService.validate')  # Method doesn't exist
    def test_ollama_provider_down_error_handling(self, mock_validate, client):
        """Test error handling when ollama service is down"""
        mock_validate.side_effect = Exception("Connection refused")
        ...
        assert "connection" in data["detail"].lower() or "provider" in data["detail"].lower()
...
class TestQuickstartJourney3_OpenAIIntegration:
    ...
    # @patch('src.services.openai_service.OpenAIService.validate')  # Method doesn't exist
    def test_openai_invalid_api_key(self, mock_validate, client):
        ...
        data = response.json()
        assert "error" in data
        assert "api key" in data["detail"].lower() or "authentication" in data["detail"].lower()
    ...
    def test_openai_quality_comparison(self, mock_openai, mock_provider_factory, client, sample_puzzle_words):
        ...
        # OpenAI should provide detailed explanations
        assert len(data["connection_explanation"]) > 50
...
class TestQuickstartJourney4_ErrorScenarios:
    ...
    def test_insufficient_words_error(self, client):
        ...
        data = response.json()
        assert "error" in data
        assert "words" in data["detail"].lower() or "insufficient" in data["detail"].lower()
...
class TestQuickstartJourney5_CompleteGameFlow:
    ...
    def test_mixed_provider_usage(self, mock_ollama, mock_provider_factory, client, sample_puzzle_words):
        ...
        # First request with simple provider
        simple_request = {
            "remaining_words": sample_puzzle_words,
            "completed_groups": [],
            "previous_guesses": [],
        ...
        assert simple_response.json()["provider_used"] == "simple"
        
        # Second request with ollama provider
        ollama_request = {
            "remaining_words": sample_puzzle_words[4:],  # Remove first 4 words
            "completed_groups": [{"words": sample_puzzle_words[:4], "category": "Fish"}],
            "previous_guesses": [sample_puzzle_words[:4]],
            "total_mistakes": 0,
            "max_mistakes": 4,
            "provider_type": "ollama",
            "model_name": "llama2"
        ...
        assert ollama_response.json()["provider_used"] == "ollama"
...
class TestAPIContractCompliance:
    ...
    def test_recommendation_response_schema(self, client, sample_puzzle_words):
        ...
        required_fields = [
            "recommended_words", "connection_explanation", "confidence_score",
            "provider_used", "puzzle_state", "alternative_suggestions"
        ]
