...
## GHCP Prompts used with CALM architecture files

### Update the CALM architecture artifacts from current #codebase
...
### Review CALM architecture artifacts for completeness
...
### Request GHCP to address any gaps in the documentation
Fill in high priority metadata gaps from the code base (GHCP Agent Mode, Claude Sonnet 4.5)
```text
from the #codebase can you fill in the high priority gaps. If the information is not in the #codebase, state "Unknown".
```

### Generate Markdown document from CALM architecure json file (better diagrams)

Generated by this GHCP prompt with VSCode Agent Mode and Claude Sonnet 4.5 LLM.  Before prompting GHCP, attached CALM `docs/calm/connection_solver.architecture.json` to the GHCP chat context:

```text
create markdown document called docs/diagrams/connection_solver_architecture.md containing mermaid diagram depicting nodes and interfaces from the calm architecture file
```
