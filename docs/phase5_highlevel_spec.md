...
phase5 db support — Functional Specification
...
When a puzzle session is created and played, the server-side session manager maintains the following attributes at minimum:
- `puzzle_id`: a deterministic identifier derived from the session's 16 puzzle words (see implementation notes for hashing rules). This identifier is used to group identical puzzles across sessions.
...
For implementation-level details (SQL DDL, exact API JSON examples, full CSV sample, exact hashing algorithm including SHA1, header strings, and code-level guidance), see `docs/phase5_implementation_notes.md`.
